...
comparacion_precios <- detalle %>%
  ...
  arrange(ProductID) %>%
  filter(!duplicated(select(., ProductName, Diferencia))) %>%  # Elimina duplicados con la misma diferencia y el mismo nombre
  distinct(ProductID, .keep_all = TRUE)  # Elimina filas duplicadas basadas en ProductID
...
# Gráfico para las diferencias más altas
ggplot(top_10_altas, aes(x = reorder(ProductName, Diferencia), y = Diferencia)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotar nombres si es necesario

# Gráfico para las diferencias más bajas
